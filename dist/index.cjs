...
const MAX_BITRATE_PER_TIER = {
  [discord_js.GuildPremiumTier.None]: 64e3,
  [discord_js.GuildPremiumTier.Tier1]: 128e3,
  [discord_js.GuildPremiumTier.Tier2]: 256e3,
  [discord_js.GuildPremiumTier.Tier3]: 384e3
};
function fetchChannelPermissions(channel) {
  const permissions = [];
  channel.permissionOverwrites.cache.filter((permission) => permission.type == discord_js.OverwriteType.Role).forEach((permission) => {
...
function fetchVoiceChannelData(channel) {
  return {
    type: discord_js.ChannelType.GuildVoice,
    name: channel.name,
...
async function fetchStageChannelData(channel, options, limiter) {
  const channelData = {
    type: discord_js.ChannelType.GuildStageVoice,
    name: channel.name,
    nsfw: channel.nsfw,
...
async function fetchTextChannelData(channel, options, limiter) {
  const channelData = {
    ...
    name: channel.name,
    nsfw: channel.nsfw,
    rateLimitPerUser: channel.type == discord_js.ChannelType.GuildText ? channel.rateLimitPerUser : void 0,
    parent: channel.parent ? channel.parent.name : null,
    topic: channel.topic,
    permissions: fetchChannelPermissions(channel),
    messages: [],
    isNews: channel.type == discord_js.ChannelType.GuildAnnouncement,
    threads: []
  };
...
async function loadCategory(categoryData, guild, limiter) {
  const category = await limiter.schedule({ id: `loadCategory::guild.channels.create::${categoryData.name}` }, () => guild.channels.create({ name: categoryData.name, type: discord_js.ChannelType.GuildCategory }));
...
async function loadChannel(channelData, guild, category, options, limiter) {
  ...
  if (channelData.type == discord_js.ChannelType.GuildText || channelData.type == discord_js.ChannelType.GuildAnnouncement) {
    ...
    createOptions.type = channelData.isNews && guild.features.includes(discord_js.GuildFeature.News) ? discord_js.ChannelType.GuildAnnouncement : discord_js.ChannelType.GuildText;
  } else if (channelData.type == discord_js.ChannelType.GuildVoice) {
  ...
  } else if (channelData.type == discord_js.ChannelType.GuildStageVoice) {
    let bitrate = channelData.bitrate;
    const bitrates = Object.values(MAX_BITRATE_PER_TIER);
    while (bitrate > MAX_BITRATE_PER_TIER[guild.premiumTier]) {
      bitrate = bitrates[guild.premiumTier];
    ...
    createOptions.userLimit = channelData.userLimit;
    createOptions.type = channelData.type;
    if (!guild.features.includes(discord_js.GuildFeature.Community))
      return null;
  }
  ...
  if (channelData.type == discord_js.ChannelType.GuildText) {
    let webhook;
...
async function clearGuild(guild, limiter) {
  ...
  await limiter.schedule({ id: "clearGuild::guild.setDefaultMessageNotifications" }, () => guild.setDefaultMessageNotifications(discord_js.GuildDefaultMessageNotifications.OnlyMentions));
  await limiter.schedule({ id: "clearGuild::guild.setWidgetSettings" }, () => guild.setWidgetSettings({ enabled: false, channel: null }));
  if (!guild.features.includes(discord_js.GuildFeature.Community)) {
    await limiter.schedule({ id: "clearGuild::guild.setExplicitContentFilter" }, () => guild.setExplicitContentFilter(discord_js.GuildExplicitContentFilter.Disabled));
    await limiter.schedule({ id: "clearGuild::guild.setVerificationLevel" }, () => guild.setVerificationLevel(discord_js.GuildVerificationLevel.None));
  }
  ...
  await limiter.schedule({ id: "clearGuild::guild.setSystemChannelFlags" }, () => guild.setSystemChannelFlags([
    discord_js.GuildSystemChannelFlags.SuppressGuildReminderNotifications,
    discord_js.GuildSystemChannelFlags.SuppressJoinNotifications,
    discord_js.GuildSystemChannelFlags.SuppressPremiumSubscriptions
  ]));
...
async function getChannels(guild, options, limiter) {
  ...
  const categories = channels.filter((channel) => channel.type == discord_js.ChannelType.GuildCategory).sort((a, b) => a.position - b.position).toJSON();
  for (let category of categories) {
    ...
    for (let child of children) {
      let channelData;
      if (child.type == discord_js.ChannelType.GuildText || child.type == discord_js.ChannelType.GuildAnnouncement) {
        channelData = await fetchTextChannelData(child, options, limiter);
      } else if (child.type == discord_js.ChannelType.GuildVoice) {
  ...
  const others = channels.filter((channel) => {
    return !channel.parent && channel.type != discord_js.ChannelType.GuildCategory && channel.type != discord_js.ChannelType.AnnouncementThread && channel.type != discord_js.ChannelType.PrivateThread && channel.type != discord_js.ChannelType.PublicThread;
  }).sort((a, b) => a.position - b.position).toJSON();
  for (let channel of others) {
    let channelData;
    if (channel.type == discord_js.ChannelType.GuildText || channel.type == discord_js.ChannelType.GuildAnnouncement) {
...
async function loadConfig(guild, backup, limiter) {
  ...
  const changeableExplicitLevel = guild.features.includes(discord_js.GuildFeature.Community);
...
async function loadAFk(guild, backup, limiter) {
  if (backup.afk) {
    try {
      await limiter.schedule({ id: "loadAFK::guild.setAFKChannel" }, () => guild.setAFKChannel(guild.channels.cache.find((channel) => channel.name == backup.afk.name && channel.type == discord_js.ChannelType.GuildVoice)));
...
let backups = `${__dirname$1}/backups`;
...
async function create(guild, options = {}) {
  const intents = new discord_js.IntentsBitField(guild.client.options.intents);
  if (!intents.has(discord_js.GatewayIntentBits.Guilds))
  ...
  const backup = {
    ...
    guildID: guild.id,
    id: options.backupId ?? discord_js.SnowflakeUtil.generate(Date.now())
  };
